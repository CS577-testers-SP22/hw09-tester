...
def run_tests(tests):
    for test_name, test_case in tqdm(tests.items()):
        user_output, user_error = shell('make run', stdin=test_case['input'])
        user_output = '\n'.join(user_output.split('\n')[1:]) # first line is Makefile's run command
        if user_output != test_case['output']:
            ...
            diff = list(difflib.ndiff(user_output, test_case['output']))
            whitespace = {'\n':'"\\n" (newline)', '\t':'"\\t" (tab)', ' ':'" " (space)'}
            ...
            if not all(x[0] == ' ' or x[-1] in whitespace for x in diff):
                ...
                sys.stdout.writelines(difflib.unified_diff(user_output.splitlines(keepends=True), 
                                                           test_case['output'].splitlines(keepends=True), 
...
def get_versions():
    ...
    try:
        with urlopen('https://raw.githubusercontent.com/CS577-testers-SP22/hw09-tester/master/.version') as f:
            if f.status != 200:
                raise Exception
...
if __name__ == '__main__':
    print(f'Running CS577 SP22 HW09 tester v{__version__}')
    ...
    if current < latest:
        ...
        print('You can download the latest version at https://github.com/CS577-testers-SP22/hw09-tester\n')
    print()
    main()
    print()
